...
public class CircleViewModel extends BaseViewModel {
    ...
    public void start(String type, User targetUesr) {
        requestDynamicList(0, type, targetUesr);
    }

    public void refresh(String type, User targetUser) {
        mDataHelper.clear();
        requestDynamicList(0, type, targetUser);
    }

    public void selectCircle(String type) {

        mNotifySwipeRefresh.postValue(true);
        mDataHelper.clear();
        requestDynamicList(0, type, null);
    }

    private void requestDynamicList(final int offset, String type, User targetUser) {
        ...
        if (!TextUtils.isEmpty(type) && !TextUtils.equals(type, "全部")) {
...
//                    updateDynamicCommentCount(dynamic);
    ...
    private void updateDynamicCommentCount(Dynamic dynamic) {
    ...
    public void next(String circleType, User targetUser) {
        if (mDataHelper.isHasMore()) {
            requestDynamicList(mDataHelper.getOffset(), circleType, targetUser);
        } else {
            mNotifyFooter.postValue(false);
        }
    }
